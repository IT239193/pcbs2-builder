...
<div id="popupContainer">
    ...
    <div class="popup-filters">
        ...
        <input type="text" id="popupSearch" placeholder="🔍 Search part by name..." oninput="refreshPopup()">
...
<div class="input-grid">
    ...
    <div class="input-group">
        ...
        <input type="text" id="minTargetScore" value="10,000">
    </div>

    <!-- RENAMED: Target Score -> Max Score -->
    <div class="input-group">
    ...
    <div class="input-group">
        <label>Max Results Limit (for Display)</label>
        <input type="text" id="resLimit" value="2,000">
    </div>
    <div class="input-group">
        ...
        <div class="results-meta">
            ...
            <div class="sort-controls">
                ...
                <div class="toggle-wrapper">
                    ...
                    <div class="toggle-btns">
                        <button id="btnXMP_Off" class="toggle-btn active" onclick="setToggle('xmp', false)">OFF</button>
                        <button id="btnXMP_On" class="toggle-btn" onclick="setToggle('xmp', true)">ON</button>
                    </div>
                </div>
    ...
    <div id="dataView" style="display: none;">
        ...
        <input type="text" id="dataSearchBox" class="dropdown-search" style="margin-bottom:20px; padding:15px; font-size:1.1em;" placeholder="Type to filter components list..." oninput="filterDataList()">
    ...
    function renderDataList() {
        ...
        items.forEach(item => {
            ...
            if (currentDataTab === 'gpu') {
                ...
                const freqStr = `${item.baseFreq || "?"} / ${item.freqMid || "?"} / ${item.maxFreq || "?"}`;
...
function calculateCpuScore(cpu, ramOrFreq, stickCount, mode, useOC, useXMP, moboLimit = 0) {
    ...
    async function generateBuilds() {
        ...
        const config = {
            ...
            // NEW: Min and Max Range Logic
            minGenScore: parseVal('minTargetScore'), 
            maxGenScore: parseVal('targetScore'), // The old "Target Score" input is now the Max
            
            lvl: parseInt(document.getElementById('level').value),
            maxPoolSize: parseIntVal('resLimit'),
        ...
        // --- 2. PREPARE AUX LISTS ---
        // For Cases: If Optimization is OFF, we need the raw list so the Assembler can iterate them.
        let rawCases = Array.from(DB.cases.values()).filter(c=>c.level<=config.lvl);
        
        const lists = {
            allCases: rawCases.sort((a,b)=>a.price-b.price),
            allCoolers: config.useCooler ? Array.from(DB.cooler.values()).filter(c=>c.level<=config.lvl).sort((a,b)=>a.price-b.price) : [],
            allPsu: config.usePsu ? Array.from(DB.psu.values()).filter(p=>p.level<=config.lvl).sort((a,b)=>a.price-b.price) : [],
            allFans: config.useFan ? Array.from(DB.fan.values()).filter(f=>f.level<=config.lvl).sort((a,b)=>a.price-b.price) : []
        };
        ...
        selectedOptions.ssdNames.forEach((count, name) => {
            ...
            if (part && part.level <= config.lvl) { 
                for(let i=0; i<count; i++) { finalStorageList.push(part); totalStorageCost += part.price; requiredSSDSlots++; }
            }
        });
        selectedOptions.hddNames.forEach((count, name) => {
            const part = DB.storage.get(name);
            if (part && part.level <= config.lvl) { 
                for(let i=0; i<count; i++) { finalStorageList.push(part); totalStorageCost += part.price; requiredHDDSlots++; }
            }
        });

        if (config.useStorage && finalStorageList.length === 0) {
            const fallback = Array.from(DB.storage.values()).filter(s => s.type !== 'HDD' && s.level <= config.lvl).sort((a,b) => a.price - b.price)[0];
            if (fallback) {
        ...
        lists.reqSSDSlots = requiredSSDSlots;
        lists.reqHDDSlots = requiredHDDSlots;
        ...
        if (OPTIMIZE_BUILD_RESULTS) {
            ...
            // RAW LIST: Filter by constraints, but keep duplicates/variants
        ...
        // --- 7. MOTHERBOARD LIST (RAW VS OPTIMIZED) ---
...
function runScanChunk(cpuIndex, config, lists) {
        ...
        // A. Get Valid RAMs and their Frequencies (For XMP ON)
        ...
        // B. Get Valid Motherboards and their Default Speeds (For XMP OFF)
        const cpuMobos = config.useMobo ? lists.validMobos.filter(m => m.socket === cpu.socket) : [null];
        
        // Extract unique 'Default Memory Speed' values from valid motherboards
        const uniqueDefaultSpeeds = [...new Set(
            cpuMobos
            .filter(m => m !== null)
            .map(m => m.defaultMem)
        )].sort((a,b) => a - b);
        ...
        for (const gpuConf of precalcGpuConfigs) {
            ...
            for (const sticks of possibleSticks) {
                ...
                // =========================================================
                // CORE LOGIC: CHOOSE WHICH LIST OF SPEEDS TO ITERATE
                // =========================================================
                ...
                for (const freq of freqsToTest) {
                    if (foundWorkingSpec) break; 

                    // 4. CALCULATE SCORE (Virtual Test)
                    ...
                    //  MIN / MAX RANGE CHECK
                    // ===============================================
                    if (totalScore < config.minGenScore || totalScore > config.maxGenScore) {
                        continue; // Skip if outside the defined range
                    }
                    
                    foundWorkingSpec = true;

                    // 5. FIND COMPONENTS THAT MATCH THIS FREQUENCY
                    
                    // A. Find RAM
                    const validRams = availableRams.filter(r => {
                        // Logic: RAM must be fast enough to run at 'freq'
                        if (r.freq < freq) return false;
                        
                        // Compatibility
                        const memType = r.type;
                        if (!cpu.memTypes.includes(memType)) return false; 
                    ...
                    const validMobos = cpuMobos.filter(m => {
                        if (!m) return true; // No mobo flow
                        if (validRams[0].type !== m.ramType) return false; // Match RAM Type
                        ...
                        if (config.useXMP) {
                            // XMP ON: Mobo must handle this speed
                            if (m.maxMemSpeed < freq) return false; 
                        } else {
                            // XMP OFF: Mobo's default speed MUST be exactly the freq we just tested
                            if (m.defaultMem !== freq) return false;
                        }
                        return true;
                    });

                    // C. ASSEMBLE (Use helper to handle Case/PSU/Cooler expansion)
                    const ramCap = OPTIMIZE_BUILD_RESULTS ? 3 : validRams.length;
                    
                    assembleBuilds(
                        validRams.slice(0, ramCap), 
                        validMobos, 
                        cpu, gpuConf, sticks, totalScore, liveCpuScore, config, lists, stmt
                    );
                }
            } 
        }
...
function assembleBuilds(rams, mobos, cpu, gpuConf, sticks, score, cpuScore, config, lists, stmt) {
        for (const ram of rams) {
            for (const mobo of mobos) {
                ...
                let validCases = [];
                if (config.useCase) {
                    validCases = lists.allCases.filter(c => {
                    ...
                    // Check Global Optimization Variable
                    if (OPTIMIZE_CASE_SELECTION && validCases.length > 0) validCases = [validCases[0]];
                } else {
                    validCases = [null];
                }

                for (const bCase of validCases) {
                    let currentPrice = corePrice + (bCase ? bCase.price : 0);
                    
                    let cooler = null; 
                    if (config.useCooler) {
                        cooler = lists.allCoolers.find(c => checkFit('cooler', c, {cpu, case: bCase}));
                        if(cooler) currentPrice += cooler.price;
                    }

                    let psu = null;
                    if (config.usePsu) {
                        psu = lists.allPsu.find(p => p.watts >= totalWatts && (!bCase || bCase.maxPsuLen === 0 || p.length <= bCase.maxPsuLen));
                        if(psu) currentPrice += psu.price;
                    }
                    
                    let fan = null;
                    if (config.useFan && bCase) {
                        fan = lists.allFans.find(f => checkFit('fan', f, {case: bCase}));
                    ...
                    // Insert into DB. 
                    // Diff is calculated based on the Max Score (the Upper Limit).
                    stmt.run([
                        score, cpuScore, gpuConf.score, 
                        score - config.maxGenScore, // Calculate Diff against Max Score
                        currentPrice, totalWatts, cpu.name, gpuConf.gpu.name, gpuConf.count,
                        ram.name, sticks, mobo ? mobo.name : null, bCase ? bCase.name : null,
    ...
    function renderPage(page) {
        ...
        builds.forEach((b, i) => {
            ...
            const buildIdx = i; // Index relative to current page for popup

            const gpuCountStr = b.gpuCnt > 1 ? `${b.gpuCnt}x ` : '';
            const gpuEmoji = b.gpu.isWater ? '💧' : '💨';
            ...
            let cpuSpecs = "";
            if (currentMode === 'TSX') {
                cpuSpecs = `Base Score (TSX): ${formatNumber(displayCpuScore)} | ${b.cpu.socket} | ${b.cpu.cores}C`;
            } else if (useOC) {
                cpuSpecs = `OC Score: ${formatNumber(displayCpuScore)} | ${b.cpu.socket} | ${b.cpu.cores}C | ${b.cpu.maxFreq}MHz`;
            } else {
                cpuSpecs = `Base Score: ${formatNumber(displayCpuScore)} | ${b.cpu.socket} | ${b.cpu.cores}C`;
            }
            ...
            if (b.ram) {
                ...
                <div class="part-row brand-ram text-colored">
                    ...
                    <div class="part-specs">${b.ram.freq} MHz | ${b.ram.type} | Total: ${b.ram.size * b.ramSticks}GB</div>
                </div>`;
            }
            ...
            if (b.mobo) {
                ...
                <div class="part-row brand-aux text-colored">
                    ...
                    <div class="part-specs">${b.mobo.socket} | ${b.mobo.size} | PCI: ${b.mobo.pciSlots}</div>
                </div>`;
            }
            if (b.psu) {
                auxHTML += `
            ...
            if (b.case) {
                interactiveHTML += renderInteractive('Case', b.case, buildIdx, 'cases',
                    `Max GPU: ${b.case.maxGpuLen}mm | Size: ${b.case.size}`
                );
            }
            if (b.cooler) {
                interactiveHTML += renderInteractive('Cooler', b.cooler, buildIdx, 'cooler',
                    `${b.cooler.type} | Flow: ${b.cooler.flow} CFM`
                );
            }
            if (b.fan) {
            ...
            <div class="build-card" style="animation-delay:${i*0.02}s">
                <div class="card-header">
                    ...
                    <div class="header-stats">
                        <div class="stats-row">
                            <div class="watts-box">⚡ ${b.watts}W</div>
    ...
    function refreshPopup() {
        ...
        subset.forEach((p, idx) => {
            ...
            if (type === 'cooler') details = `${p.type} | Flow: ${p.flow}`;
